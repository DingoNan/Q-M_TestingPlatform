            ...
            添加步骤
			  ...
			  <template #dropdown>
			    <el-dropdown-menu class="control-menu">
			      ...
			      <!-- ★ 2026-09-17 隐藏「App自动化」：本公司被测产品基本没有 App 侧测试，
			           该入口是噪音。原条件为 v-if='case_info.type === 3'，
			           改为 false 保留节点结构，日后需要时把条件还原即可。
			           注意：本文件有「步骤抽屉」「用例维护主页面」两处同名下拉菜单，
			           两处必须同步隐藏，否则会出现一个入口消失、另一个仍在的错乱。 -->
			      <el-dropdown-item divided :command="7" class="control-item" v-if='false'>
				  ...
				  <el-row :gutter="20" v-if="one_step_obj.type === StepType.Request || one_step_obj.com_step_type === StepType.Request">
				   ...
				   <el-col :span="4" style="padding-left: 40px; padding-right: 0px">
					  ...
					  <el-select v-model='one_step_obj.api_method'  size='large' class="select" popper-class='select-dropdown-rounded' style='margin-left: 15px'>
					  	<el-option color="#f59e0b" size="large" effect="dark" class="method-tag" label='GET' value="GET" v-if='one_step_obj.api_method === "GET"'></el-option>
			  ...
			  <el-collapse-item  name="4" >
			    ...
			    <el-tabs v-model="one_step_obj.step_active_tab" class="elegant-tabs">
			      ...
			      <el-tab-pane label="请求体" name="body" v-if="one_step_obj.type === StepType.Request || one_step_obj.com_step_type === StepType.Request">
			       <el-row style="display: flex; align-items: center; gap: 30px; margin-bottom: 20px; padding: 16px; background-color: var(--qm-bg-1); border-radius: 8px; border: 1px solid var(--qm-line);">
			         <div style="display: flex; align-items: center; gap: 15px;">
			           <span style="font-weight: 600; color: var(--qm-text-1); width: 80px;">请求体类型</span>
			         ...
			         <div v-if="one_step_obj.body_type === 1" style="display: flex; align-items: center; gap: 15px; padding-left: 20px; border-left: 1px solid #dcdfe6;">
			           <span style="font-weight: 600; color: var(--qm-text-1); width: 100px;">JSON根类型</span>
			           <el-select 
			  ...
			  <div  v-if="one_step_obj.body_type === 1">
				  <el-row>
					  <div style="display: flex; align-items: center; gap: 15px; padding-bottom: 20px;padding-left: 20px; border-left: 1px solid #dcdfe6;">
						  <span style="font-weight: 600; color: var(--qm-text-1); width: 100px;">数据根类型</span>
						  <el-select 
							v-model="one_step_obj.api_json_type" 
							class="select" size='large' popper-class='select-dropdown-rounded'
							style="width: 150px;"
		  ...
		  <el-card class='step_item_card' v-if="one_step_obj.type !== StepType.Control && one_step_obj.com_step_type !== StepType.Control">
			  ...
			  <el-tabs  class="elegant-tabs">
			    ...
			    <el-tab-pane lazy="true"  label="JSON 文档结构校验" v-if="one_step_obj.type === StepType.Request || one_step_obj.com_step_type === StepType.Request">
			  	  <el-row style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; padding: 0 10px; background-color: var(--qm-bg-1); border-radius: 8px; height: 70px;">
...
/**
 * ★ 步骤对象模板（模块级，2026-09-17 新增）
 *
 * 原先是直接写在 data() 里的对象字面量。问题出在
 * controlStepCommand / controlCommand 里用 `{ ...this.one_step_obj_tmp }`
 * 整体替换 one_step_obj，而 one_step_obj_tmp 的初始值是 {}，
 * 于是 api_json / api_response / api_data / api_headers / api_params
 * 这些字段全部丢失变成 undefined。
 *
 * 保存时 remove_vxe_table_info() 执行
 *   JSON.stringify(undefined) → 返回 undefined（注意：不是字符串 "undefined"）
 *   JSON.parse(undefined)     → 抛 SyntaxError: "undefined" is not valid JSON
 * 该异常无 try/catch，直接中断 saveStep，CreateStep/UpdateStep 永不执行，
 * 用户看到的现象就是「保存 / 保存并关闭 按钮点了没反应」。
 *
 * 抽成模块级工厂函数（而不是挂在 methods 上），是为了让 data() 里也能安全调用，
 * 不受 Vue 选项初始化时序影响。
 */
function stepObjTemplate() {
  return {
    id: 0,
    parent_id: null,
    step_index: '',
...
export default {
  ...
  methods: {
    ...
    /**
     * ★ 安全展开折叠面板（2026-09-17 新增）
     * `this.$refs.collapse` 在 el-collapse 因 v-if 未渲染时是 undefined，
     * 旧代码直接 .setActiveNames('3') 会抛 TypeError 并中断整个方法
     * （controlStepCommand 中断后，页面停留在残缺状态，后续保存必崩）。
     */
    setCollapseActive(name) {
      const c = this.$refs && this.$refs.collapse
      if (c && typeof c.setActiveNames === 'function') {
        c.setActiveNames(name)
      }
    },
    ...
    controlCommand(command) {
      this.stepView = false
      this.loading = false
      // ★ 同 controlStepCommand：保证默认字段不丢（见 STEP_OBJ_TEMPLATE 注释）
      this.one_step_obj = { ...this._defaultStepObj(), ...this.one_step_obj_tmp }
	...
	controlStepCommand(command) {
	  this.stepView = false
	  this.loading = false
	  // ★ 2026-09-17 修复：one_step_obj_tmp 初始为 {}，直接整体替换会把默认的
	  //   api_json / api_response / api_data 等字段全部丢掉（变成 undefined），
	  //   保存时 JSON.parse(JSON.stringify(undefined)) 抛 SyntaxError。
	  //   这里改为「先取默认对象，再用 _tmp 覆盖」，保证字段始终齐全。
	  this.one_step_obj = { ...this._defaultStepObj(), ...this.one_step_obj_tmp }
  ...
  //       this.one_step_obj.api_json = this.one_step_obj.api_all.json
    ...
    remove_vxe_table_info() {
      // ★ 防御性归一化（2026-09-17 修复「保存按钮点了没反应」）：
      // 旧实现在 api_json / api_response 为 undefined 时，
      // JSON.stringify(undefined) 返回的是 undefined 而不是字符串，
      // 紧接着 JSON.parse(undefined) 抛 SyntaxError: "undefined" is not valid JSON，
      // 该异常无 try/catch，直接中断 saveStep，导致 CreateStep/UpdateStep 永远不执行，
      // 用户看到的现象就是「保存 / 保存并关闭 点了一直没反应」且没有任何提示。
      // 根因是 one_step_obj 被整体替换过（controlStepCommand 里 { ...one_step_obj_tmp }，
      // 而 one_step_obj_tmp 初始为 {}），对象上的 api_* 字段全部丢失变成 undefined。
      // 这里统一兜底成数组，保证后续 .length / for...in / JSON 序列化都不再炸。
      const _arr = (v) => (Array.isArray(v) ? v : [])
      this.one_step_obj.api_json = _arr(this.one_step_obj.api_json)
      this.one_step_obj.api_response = _arr(this.one_step_obj.api_response)
      this.one_step_obj.api_data = _arr(this.one_step_obj.api_data)
      this.one_step_obj.api_headers = _arr(this.one_step_obj.api_headers)
      this.one_step_obj.api_params = _arr(this.one_step_obj.api_params)
      ...
      for (let index = this.one_step_obj.api_json.length - 1; index >= 0; index--) {
        ...
        if (this.one_step_obj.api_json_tree[index] && this.one_step_obj.api_json_tree[index].parentId !== null) {
      ...
      for (let index = this.one_step_obj.api_response.length - 1; index >= 0; index--) {
        ...
        if (this.one_step_obj.api_response_tree[index] && this.one_step_obj.api_response_tree[index].parentId !== null) {
    ...
    delExtraInfo(data) {
      // 同上：入参不是数组时直接返回，避免 for..length 抛异常
    ...
    /**
     * ★ 真正执行保存（2026-09-17 新增）
     * 把 remove_vxe_table_info + Create/Update 收进 try/catch。
     * 旧代码这两步是裸调用的，一旦抛异常（如 JSON.parse(undefined) 的 SyntaxError）
     * 会直接冒泡出 saveStep 并被 Vue 事件循环吞掉：
     * 既没有网络请求，也没有任何错误提示，用户只看到「点了没反应」。
     * 现在捕获后 ElMessage 明确告知，便于后续定位，不再静默失败。
     */
    _doSaveStep(is_close) {
      try {
        this.remove_vxe_table_info()
      } catch (e) {
        console.error('[保存步骤] 预处理参数失败:', e)
        ElMessage({
          type: 'error',
          duration: 6000,
          showClose: true,
          message: '保存前的参数整理出错：' + (e && e.message ? e.message : e) + '（请联系管理员）'
        })
        return
      }
    ...
    async getStep(case_id, step_id, case_step_id, set_id_zero = false, is_com_step = false, is_update_step_index = false) {
      ...
      if (response.status === 200) {
        // ★ 2026-09-17：整体替换前先垫上默认对象，避免后端某字段缺失时
        //   前端读到 undefined（后续 remove_vxe_table_info 会因此抛 SyntaxError）。
        this.one_step_obj = { ...this._defaultStepObj(), ...(response.data.result || {}) }
...
.back-btn:hover {
  background: var(--qm-bg-1);
  border-color: var(--qm-line-strong) !important;
...
.run-switch {
  ...
  --el-switch-off-color: var(--qm-text-3);
}
...
.elegant-dialog :deep(.el-dialog) {
  ...
  background: linear-gradient(135deg, var(--qm-bg-2) 0%, var(--qm-bg-1) 100%) !important;
...
.config-item {
  ...
  margin-bottom: 10px;
  background-color: var(--qm-bg-2);         /* 背景色，可选 */
...
.label {
  ...
  white-space: nowrap;
  color: var(--qm-text-2);           /* 更深的灰色，接近黑色 */
}
...
.elegant-drawer .step-collapse :deep(.el-collapse-item__header) {
  ...
  /* 保持原有样式 */
  background: var(--qm-bg-2) !important;
  border: 1px solid var(--qm-line) !important;
