<template>
  ...
  <!-- 用例编辑对话框 -->
  <el-dialog v-model="editCaseVisible" :title="title" width="600" :close-on-click-modal='false' class="elegant-dialog">
    <div class="dialog-content">
      <el-form :model="caseForm" label-position='top' :disabled='caseView' :rules="caseRules" ref='caseRef' class="dialog-form">
        <el-form-item  prop="name" class="dialog-form-item">
          ...
          <el-input v-model="caseForm.name" autocomplete="off" placeholder="请输入用例名称" class="input" size='large' maxlength="50" show-word-limit/>
        </el-form-item>
        
		<el-form-item prop='module' class="dialog-form-item" style="display: flex; flex-direction: column; gap: 8px; align-items: flex-start;">
		  <label class="dialog-label" style="display: flex; align-items: center; font-weight: 500;">
        ...
        <el-form-item  prop="tag" class="dialog-form-item">
          <label class="dialog-label">
            <i class="icon-tag-dialog"></i>
            用例标签
			<el-link type="success"  style='margin-left: 330px;' @click="() => this.$router.push({ name: 'tag' })">去添加用例标签</el-link>
          </label>
          <el-select v-model="caseForm.tag" placeholder="请选择用例标签" clearable multiple class="select" size='large' popper-class='select-dropdown-rounded'  filterable >
        ...
        <div class="content-header">
          <div class="content-title-section">
            <div class="title-with-stats">
              ...
              <div class="stats-info">
                ...
                <div class="stat-item type-badge" v-if="caseSearch.type">
                  <i :class="`type-icon-small type-icon-${caseSearch.type}`"></i>
            ...
            <div class="header-action-section">
              ...
              <el-dropdown
                v-if="(permission.has_edit_permission || permission.has_delete_permission) && !isCanChoose"
...
export default{
  ...
  data() {
    return {
      ...
      // ★ 2026-09-17 隐藏「App自动化」入口：
      //   本公司被测产品基本没有 App 侧测试，该筛选项对使用者是纯噪音。
      //   注意：这里只隐藏「UI 入口」，automationTypes 数组本身保持完整不动 ——
      //   因为 getTypeLabel() / getTypeTagType() 依赖它做「值 ↔ 文案」双向映射，
      //   若直接删条目，任何历史遗留的 type=3 用例会显示成空白标签。
      //   日后需要恢复时，把 '3' 从这个数组里去掉即可。
      hiddenAutomationTypes: ['3'],
      result_list: [
        {label: '成功', value: 1},
        {label: '失败', value: 2},
        {label: '错误', value: 3},
  ...
  computed: {
    /**
     * ★ 供模板渲染的自动化类型列表（2026-09-17 新增）
     * 过滤掉 hiddenAutomationTypes 里声明的类型（当前为 App自动化 '3'），
     * 只影响顶部筛选按钮组的显示，不影响 automationTypes 的映射能力。
     */
  ...
  methods:{
	...
	chooseCaseId(caseData){
	  this.$emit('setCaseData', caseData)
  ...
  if (this.batchDialogType === 'module') {
	...
	chooseManyCase(){
		...
		this.$emit('setManyCaseData', this.multipleSelection)
	},
...
.node-content {
  ...
  max-width: calc(100% - 32px); /* 限制最大宽度，为操作按钮留出空间 */
}
...
/* 不同类型按钮的样式 */
.type-radio-group >>> .type-1 .el-radio-button__inner {
...
.type-radio-group >>> .type-3 .el-radio-button__inner {
  border-color: var(--qm-warning-line);
  background: var(--qm-warning-soft);
  color: #92400e;
}

.type-radio-group >>> .type-4 .el-radio-button__inner {
  border-color: var(--qm-bg-3);
  background: var(--qm-bg-1);
  color: #374151;
}

.type-radio-group >>> .type-5 .el-radio-button__inner {
  border-color: #fee2e2;
  background: var(--qm-red-soft);
  color: #991b1b;
}

.type-radio-group >>> .type-1 .el-radio-button__original-radio:checked + .el-radio-button__inner {
  border-color: #f59e0b;
  background: linear-gradient(135deg, var(--qm-warning-soft) 0%, var(--qm-warning-soft-2) 100%);
  color: #b45309;
  box-shadow: 0 4px 12px rgba(245, 158, 11, 0.2);
...
.type-radio-group >>> .type-2 .el-radio-button__original-radio:checked + .el-radio-button__inner {
  border-color: #10b981;
...
.type-radio-group >>> .type-3 .el-radio-button__original-radio:checked + .el-radio-button__inner {
  border-color: #f59e0b;
  background: linear-gradient(135deg, var(--qm-warning-soft) 0%, var(--qm-warning-soft-2) 100%);
  color: #92400e;
  box-shadow: 0 4px 12px rgba(245, 158, 11, 0.2);
...
.type-radio-group >>> .type-4 .el-radio-button__original-radio:checked + .el-radio-button__inner {
...
.type-radio-group >>> .type-5 .el-radio-button__original-radio:checked + .el-radio-button__inner {
  border-color: #ef4444;
...
/* ===== 批量操作下拉菜单 popper 样式（与 FunCaseList 一致） ===== */
