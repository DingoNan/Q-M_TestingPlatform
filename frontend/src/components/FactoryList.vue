<template>
	<el-dialog v-model="editCaseVisible" :title="title" width="600">
	    <el-form :model="caseForm" label-position='top' :disabled='caseView' :rules="caseRules" ref='caseRef'>
			<el-form-item label="造数用例名称" prop='name'>
				<el-input v-model="caseForm.name" autocomplete="off" placeholder="请输入造数用例名称" style="width: 960px;"/>
			</el-form-item>
			<el-form-item label="所属模块" tyle="margin-right: 0px;" prop="module">
			...
			<!-- <el-form-item label="是否设为数据工厂数据">
				<el-switch v-model="caseForm.is_data_factory" disabled/>
	    ...
	    <template #footer>
			<span class="dialog-footer" v-if='!caseView'>
				...
				<el-button type="primary" v-if='title === "新增造数用例" && permission.has_add_permission'  @click="createCase">确认</el-button>
				<el-button type="primary" v-if='title === "编辑造数用例" && permission.has_edit_permission' @click="updateCase">确认</el-button>
			</span>
	    </template>
	</el-dialog>
	<div class="common-layout" v-if='case_model==="tree"' >
			<el-container>
			  ...
			  <el-main style="padding-top: 0px; padding-left: 10px; padding-right: 0px;">
				  <div class="case_tree case">
					  <el-card style='margin-bottom: 10px;'>
					  	<el-form  :model="caseSearch" label-width="80px"  class='searchForm'>
					  		<el-row :gutter="20">
					  			<el-col :span='6'>
					  ...
					  <el-card style='margin-bottom: 10px;'>
						 ...
						 <div style="padding-top: 10px; padding-right: 33px; padding-bottom: 10px; background-color: var(--qm-bg-2); display: flex; justify-content: flex-end" class='btn_right'>
						 ...
						 <el-table :max-height='550' :data="case_list.results" class='table'  :header-row-style="headerRowStyle" :show-overflow-tooltip='true'>
						 <el-table-column label="序号" width="70" type="index" align="center"/>
						 <el-table-column label="用例名称" prop="name" width="150" align="center">
							<template #default='scope'>
								<el-link  type="primary" @click="editStepTwo(scope.row)">{{ scope.row.name }}</el-link>
							</template>
						 </el-table-column>
						 <el-table-column label="用例类型" prop="type" min-width="100" align="center"/>
						 <el-table-column label="执行次数" prop="all_run_times" width="100" align="center"/>
						 <el-table-column label="成功次数" prop="all_success_times" width="100" align="center"/>
						 ...
						 <el-table-column align="center"  :width="ACTION_COL_WIDTH" label="操作">
							 <template #default="scope">
	...
	<div class="case" v-if='case_model==="list"'>
	   				    ...
	   				    <el-row :gutter="20">
	   						...
	   						<el-col :span='6'>
	   							<el-form-item label="所属模块">
	   								<el-cascader :show-all-levels="false" collapse-tags v-model='caseSearch.module_list' :options="plant_module_list" :props="props" clearable style="width: 280px;"/>
	   							</el-form-item>
	   						</el-col>
	   						<el-col :span='6'>
	   							<el-form-item label="创建人">
	   ...
	   <el-card style='margin-bottom: 10px;'>
	   		<div style="background-color: var(--qm-bg-2); padding-bottom: 10px;">
	   				   <div style="padding-top: 10px; padding-right: 23px; padding-bottom: 10px; background-color: var(--qm-bg-2); display: flex; justify-content: flex-end" class='btn_right'>
	   				   	<el-tooltip content="切换模式">
	   				   		<el-button  @click="changeMode" :icon="Switch"></el-button>
	   				   	</el-tooltip>
	   				   	<el-button v-if='permission.has_add_permission' @click="addCase" type="primary" >新增</el-button>
	   				   ...
	   				   <el-table :max-height='550' :data="case_list.results" class='table'  :header-row-style="headerRowStyle" :show-overflow-tooltip='true'>
	   				   		<el-table-column label="序号" width="70" type="index" align="center"/>
	   				   		<el-table-column label="用例名称" prop="name" width="300" align="center">
	   				   			<template #default='scope'>
	   				   				<el-link  type="primary" @click="editStepTwo(scope.row)">{{ scope.row.name }}</el-link>
	   				   			</template>
	   				   		</el-table-column>
	   				   		<el-table-column label="用例类型" prop="type" min-width="100" align="center"/>
	   				   		<el-table-column label="造数执行次数" prop="all_run_times" width="150" align="center"/>
	   				   		<el-table-column label="造数成功次数" prop="all_success_times" width="150" align="center"/>
	   				   		<el-table-column label="创建人" prop="create_by_name"  width="100" align="center"/>
	   				   		<el-table-column label="更新人" prop="update_by_name"  width="100" align="center"/>
	   				   		<el-table-column label="创建时间" prop="create_time" sortable width="180" align="center"/>
	   				   		<el-table-column label="更新时间" prop="update_time" sortable width="180" align="center"/>
...
import { View, Delete, CopyDocument, Plus, EditPen, Switch} from '@element-plus/icons-vue'
    ...
    livePathPermission() {
      ...
      return {}
		...
		moduleSelect({ node, checked, $event }){
			localStorage.setItem('factory_node', JSON.stringify(node))
		...
		run(){
			if(!this.env_id){
				...
				if(this.case_info.type === 2 && this.caseRunForm.executor_id === ''){
					ElMessage({message: '请选择Web执行机', type: 'error'})
				}else if(this.case_info.type === 3 && this.caseRunForm.executor_id === ''){
					ElMessage({message: '请选择设备', type: 'error'})
				}else{
					this.caseRun()
				}
		...
		async getCases(){
			...
			this.caseSearch.is_data_factory = true
