<template>
  ...
  <div>
     <vxe-table
          ...
          <vxe-column field="is_required" title="必填" width="70" type='checkbox' v-if='!isTest'>
          </vxe-column>
          ...
          <vxe-column field="value" title="参数值" width="400" :edit-render="{autofocus: true, placeholder: '参数值'}">
			...
			<template #edit="{ row, rowIndex }">
				 <el-upload
				    ...
				    :on-change="(file) => handleChange(file, row, rowIndex)"
					v-if="row.type === 'file'"
				  >
				    <el-button style="width: 380px;">请选择文件</el-button>
				  </el-upload>
               <FuncAndParams  :bind_case_data='bind_case_data' v-if='row.type !="file"' :value='row.value' :row='row'  :step_index='step_index' :steps='steps' :index='rowIndex' :bind_env_params='bind_env_params' :bind_global_params='bind_global_params' :case_table_data='case_params_data' :func_list='func_list' @update:value="handleUpdateValue(row, $event)"></FuncAndParams>
            </template>
          </vxe-column>
		  <vxe-column field="explain" title="参数说明" min-width="285" :edit-render="{autofocus: true, placeholder: '参数名称'}">
		    <template #edit="{ row }">
		  ...
		  <vxe-column title="操作" width="163" header-align="left">
			  <template #header>
...
export default {
  ...
  methods: {
	async handleChange(file, row, rowIndex) {
	   const f = file.raw;
	   ...
	   this.tableData[rowIndex].value = {name: file.name, raw: base64, type: file.raw.type, toString() { return this.name}}
	},
	...
	deleteExtraData(){
		for (const key in this.caseForm.steps) {
			for (const index in this.caseForm.steps[key].api_json){
				delete this.caseForm.steps[key].api_json[index].children
				delete this.caseForm.steps[key].api_json[index]._X_ROW_CHILD
				delete this.caseForm.steps[key].api_json[index]._X_ROW_KEY
			}
		}
	},
	typeChange(row){
	...
	_exportJsonFuncTwo(parentId, dataObj) {
	  ...
	  for (const key in dataObj) {
	      ...
	      if (dataType === 'object' && value !== null) {
			...
			this.tableData.push(data)
			if(data.type === 'array'){
				this._exportJsonFuncTwo(data.id, value.slice(0, 1))
			}else{
