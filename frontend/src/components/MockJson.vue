<template>
  ...
  <div>
     <vxe-table
          ...
          <vxe-column field="type" title="参数类型" width="100" :edit-render="{autofocus: true, placeholder: '参数类型'}">
            <template #edit="{ row }">
		  ...
		  <vxe-column title="必含" width="80" type='checkbox'/>
		  <vxe-column field="check_method" title="校验方式" width="200" :edit-render="{autofocus: true, placeholder: '参数类型'}">
		    <template #edit="{ row }">
		  ...
		  <vxe-column title="操作" width="150" header-align="left">
			  ...
			  <template #default="{ row, rowIndex }">
				...
				<el-tooltip content="新增子字段">
					<el-button :icon="Connection"  @click="addSubJsonParams(row)"  circle></el-button>
				</el-tooltip>
...
export default {
  ...
  methods: {
	...
	delAllJsonParams(row){
		
		this.tableData.splice(parentIndex, this.tableData.length)
	},
	addJsonParams(row){
		this.json_params.id = uuidv4()
