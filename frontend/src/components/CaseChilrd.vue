...
export default{
  ...
  computed:{
     ...
     * 过滤掉 hiddenAutomationTypes（当前为 App自动化 'APP_UI'），
     * 只影响筛选按钮组显示，不影响 automationTypes 的映射能力。
     */
  ...
  data() {
    return {
      ...
      // ★ 2026-09-17 隐藏「App自动化」入口（同 CaseList.vue 的做法）：
      //   只隐藏 UI 筛选项，数组本身保留，避免破坏 getTypeLabel/getTypeTagType 的映射。
      hiddenAutomationTypes: ['APP_UI'],
      result_list: [
        {label: '成功', value: 1},
        {label: '失败', value: 2},
        {label: '错误', value: 3},
