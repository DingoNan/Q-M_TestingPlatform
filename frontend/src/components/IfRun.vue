     ...
     <vxe-table
          ...
          <vxe-column field="act_value" title="实际值" min-width="300" >
            <template #default="{ row, rowIndex }">
               <FuncAndParams :bind_case_data='bind_case_data' :value='row.act_value' :index='rowIndex' :step_index='step_index' :steps='steps' :bind_env_params='bind_env_params'  :bind_global_params='bind_global_params' :case_table_data='case_params_data' :row='row'  :func_list='func_list' @update:value="handleUpdateActValue(row, $event)"></FuncAndParams>
            </template>
          </vxe-column>
		  <vxe-column title="断言方式" field="method" align="center" width="200">
          ...
          <vxe-column field="exp_value" title="预期值" min-width="300" >
            <template #default="{ row, rowIndex }">
               <FuncAndParams :bind_case_data='bind_case_data' :value='row.exp_value' :step_index='step_index' :steps='steps' :index='rowIndex' :bind_env_params='bind_env_params' :bind_global_params='bind_global_params' :case_table_data='case_params_data' :row='row'  :func_list='func_list' @update:value="handleUpdateExpValue(row, $event)"></FuncAndParams>
            </template>
          </vxe-column>
		  <vxe-column title="与或非" field="andOr" align="center" width="120px">
		  	<template #default="{ row }">
		  		<el-select v-model="row.andOr" style="width: 100%;" size='large'class="select" popper-class='select-dropdown-rounded'>
