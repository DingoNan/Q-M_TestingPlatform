<template>
  <!-- 查看函数体对话框 -->
  <el-dialog 
    v-model="viewSeleniumVisiable" 
    title="查看函数体" 
    width="900"
    class="elegant-dialog selenium-dialog"
  ...
  >
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button @click="viewSeleniumVisiable = false" class="dialog-cancel-btn">
          关闭
        </el-button>
      </div>
    </template>
  ...
  <!-- 主内容区域 -->
  <div class="selenium-management-container">
    <!-- 筛选卡片 -->
    <el-card class="filter-card elegant-shadow">
      <div class="filter-header">
        <div class="header-title-section">
          <i class="icon-selenium"></i>
          <h3 class="filter-title">Selenium操作筛选</h3>
          <el-tag size="small" type="info" effect="plain">快速筛选</el-tag>
        </div>
      </div>
      
      <div class="filter-form-wrapper">
        <div class="selenium-type-section">
          <div class="type-label">
            <i class="icon-type"></i>
            <span class="inline-label-text">操作类型</span>
          </div>
          <el-radio-group 
            v-model="seleniumSearch.type" 
            class="selenium-type-group"
            @change="getSeleniumKey"
          >
            <el-radio-button 
              v-for="(value, label) in SeleniumType" 
              :key="value"
              :value="value"
    ...
    <el-card class="content-card elegant-shadow">
      <div class="content-header">
        <div class="content-title-section">
          <div class="title-with-stats">
            <h3 class="content-title">Selenium操作列表</h3>
            <div class="stats-info">
              <div class="stat-item">
                <span class="stat-label">总计</span>
                <span class="stat-value">{{ selenium_results.count || 0 }}</span>
              </div>
              <div class="stat-item">
      ...
      <!-- 操作列表区域 -->
      <div class="selenium-list-wrapper">
        <div class="selenium-list" v-if="selenium_results.results && selenium_results.results.length > 0">
          <el-card 
            v-for="obj in selenium_results.results" 
            :key="obj.id"
            class="selenium-item-card elegant-shadow"
          >
            <div class="selenium-item-header">
              <div class="item-tags">
                <el-tag 
                  type="success" 
                  effect="dark" 
            ...
            <div class="selenium-item-content">
        ...
        <div v-else class="empty-state">
          ...
          <p class="empty-text">暂无Selenium操作数据</p>
        </div>
      </div>
...
export default {
  name: 'SeleniumManagement',
  computed: {
    ...mapState(['pathPermission', 'projectInfo']),
  },
  emits: ['update:chooseSeleniumVisible', 'setSeleniumData'],
  props: {
    'isCanChoose': {
      type: Boolean,
      default: false,
  ...
  methods: {
    ...
    chooseSelenium(seleniumData) {
      this.$emit('update:chooseSeleniumVisible', false)
      this.$emit('setSeleniumData', seleniumData)
    },
    ...
    async getSeleniumKey() {
      try {
        const response = await this.$api.getSeleniumKey(Object.assign(this.seleniumSearch, this.page_size_params))
        if (response.status === 200) {
          this.selenium_results = { ...response.data }
        }
      } catch (error) {
        console.error('获取Selenium操作失败:', error)
      }
    },
  },
  created() {
...
.selenium-item-card:hover {
  transform: translateY(-4px);
  box-shadow: 0 12px 32px rgba(0, 0, 0, 0.12);
}

.selenium-item-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 16px;
...
@media screen and (max-width: 1200px) {
  ...
  .content-card .selenium-list-wrapper {
    padding: 16px 20px;
  }
...
.selenium-item-card:nth-child(1) { animation-delay: 0.1s; }
.selenium-item-card:nth-child(2) { animation-delay: 0.15s; }
.selenium-item-card:nth-child(3) { animation-delay: 0.2s; }
.selenium-item-card:nth-child(4) { animation-delay: 0.25s; }
.selenium-item-card:nth-child(5) { animation-delay: 0.3s; }
.selenium-item-card:nth-child(6) { animation-delay: 0.35s; }
.selenium-item-card:nth-child(7) { animation-delay: 0.4s; }
.selenium-item-card:nth-child(8) { animation-delay: 0.45s; }
.selenium-item-card:nth-child(9) { animation-delay: 0.5s; }
.selenium-item-card:nth-child(10) { animation-delay: 0.55s; }
</style>