     ...
     <vxe-table
          ...
          <vxe-column field="act_value" title="实际值" min-width="300" >
            <template #default="{ row, rowIndex }">
               <FuncAndParams :bind_case_data='bind_case_data' :value='row.act_value' :row='row'  :step_index='step_index' :steps='steps' :index='rowIndex' :bind_env_params='bind_env_params' :bind_global_params='bind_global_params' :case_table_data='case_params_data' :func_list='func_list' @update:value="handleUpdateActValue(row, $event)"></FuncAndParams>
            </template>
          </vxe-column>
		  ...
		  <vxe-column title="最大循环次数" field="max_loop_times" align="center" width="350px">
		  	<template #default="{ row, rowIndex }">
				<FuncAndParams :bind_case_data='bind_case_data' v-if='rowIndex ===0' :value='row.max_loop_times' :row='row' :step_index='step_index' :steps='steps' :index='rowIndex' :bind_env_params='bind_env_params' :bind_global_params='bind_global_params' :case_table_data='case_params_data'  :func_list='func_list' @update:value="handleUpdateValue(row, $event)"></FuncAndParams>
		  	</template>
		  </vxe-column>
    ...
    return {
	  ...
	  json_params:{
		...
		exp_value: '',//预期值
	  	method: '',//校验方式
		andOr: 'And' ,//与或非
		max_loop_times: 10, //最大循环次数
	  },
    }
  },
  methods: {
	handleUpdateValue(row, newValue) {
	      row.max_loop_times = newValue; // 更新父组件的数据
	},
