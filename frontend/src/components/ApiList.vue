<template>
  ...
  <div class="api-management-container">
    ...
    <el-dialog v-model="apiImportVisible" title="导入接口" width="900" draggable
      ...
      <div v-show="importStep === 0" class="step-content">
        <div class="format-grid">
          <div v-for="opt in importFormats" :key="opt.value"
               ...
               @click="onSelectImportFormat(opt.value)">
        ...
        <el-tabs v-else v-model="apiImportSet.source" class="source-tabs">
          <el-tab-pane label="文件导入" name="file">
            <el-upload drag :auto-upload="false" :on-change="onImportFileChange" :on-remove="onImportFileRemove"
                       :file-list="importFileList" :limit="1" accept=".json,.yaml,.yml,.jmx,.xml,.har"
                       class="import-uploader">
              <el-icon class="el-icon--upload"><UploadFilled /></el-icon>
              <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
              <template #tip>
                <div class="el-upload__tip">
                  <template v-if="apiImportSet.format === 'har'">
                    支持 .har 文件（Chrome / Edge 开发者工具 → 网络 → 右键导出 HAR）
                  </template>
                  <template v-else>支持 .json / .yaml / .yml / .jmx / .xml 文件，仅支持单个文件</template>
                </div>
              </template>
            </el-upload>
          </el-tab-pane>
      ...
      <div v-show="importStep === 2" class="step-content">
        <el-form :model="apiImportSet" label-position="top" :rules="importRules" ref="importRef">
          ...
          <!-- HAR 专属：认证补齐 -->
          <div v-if="apiImportSet.format === 'har'" class="har-auth-panel">
            <div class="har-auth-head">
              <el-icon class="har-auth-icon"><Key /></el-icon>
              <div>
                <div class="har-auth-title">认证补齐</div>
                <div class="har-auth-sub">
                  浏览器导出 HAR 时会剥离 Authorization 等凭据，不补齐则导入的接口无法回放
                </div>
              </div>
            </div>

            <el-radio-group v-model="harAuth.mode" class="match-mode-group har-auth-mode">
              <el-radio-button label="login">自动登录换取</el-radio-button>
              <el-radio-button label="token">手工填 Token</el-radio-button>
              <el-radio-button label="none">不补齐</el-radio-button>
            </el-radio-group>

            <template v-if="harAuth.mode === 'login'">
              <el-row :gutter="16" class="har-auth-row">
                <el-col :span="16">
                  <el-form-item label="登录接口地址">
                    <el-input v-model="harAuth.login.url" size="large" class="input"
                              placeholder="如 http://172.16.21.141:9088/api/v1/auth/login" />
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item label="请求方式">
                    <el-input v-model="harAuth.login.method" size="large" class="input" />
                  </el-form-item>
                </el-col>
              </el-row>
              <el-form-item label="登录请求体（JSON）">
                <el-input v-model="harAuth.login.json" type="textarea" :autosize="{ minRows: 2, maxRows: 5 }"
                          class="text" placeholder='{"username": "xxx", "password": "xxx"}' />
              </el-form-item>
              <el-form-item label="Token 提取路径">
                <el-input v-model="harAuth.login.tokenPath" size="large" class="input"
                          placeholder="如 data.token" />
                <div class="form-tip">支持点号路径与数组下标，例如 data.token / data.result[0].accessToken</div>
              </el-form-item>
            </template>

            <template v-else-if="harAuth.mode === 'token'">
              <el-form-item label="Token">
                <el-input v-model="harAuth.token" size="large" class="input" show-password
                          placeholder="粘贴已获取到的 token" />
              </el-form-item>
            </template>

            <el-row :gutter="16" v-if="harAuth.mode !== 'none'">
              <el-col :span="10">
                <el-form-item label="注入的请求头名">
                  <el-input v-model="harAuth.headerName" size="large" class="input" placeholder="Authorization" />
                </el-form-item>
              </el-col>
              <el-col :span="14">
                <el-form-item label="值模板">
                  <el-input v-model="harAuth.template" size="large" class="input"
                            placeholder="Bearer {token}" />
                  <div class="form-tip">模板中的 {token} 占位符会被替换为实际 token；不需要前缀时直接填 {token}</div>
                </el-form-item>
              </el-col>
            </el-row>

            <div class="har-preview-bar">
              <el-button @click="previewHar" :loading="harPreviewLoading" class="dialog-cancel-btn">
                <el-icon style="margin-right:4px"><View /></el-icon>解析预览
              </el-button>
              <span class="har-preview-tip">建议先预览，确认接口清单与动态参数识别结果后再导入</span>
            </div>
          </div>
        </el-form>
      </div>
    ...
    <!-- HAR 解析预览抽屉 -->
    <el-drawer v-model="harPreviewVisible" title="HAR 解析预览" size="72%" class="har-preview-drawer">
      <div v-if="harPreview" class="har-preview-body">
        <!-- 概览 -->
        <div class="har-summary">
          <div class="har-sum-item">
            <div class="har-sum-num">{{ harPreview.stats.xhr_count }}</div>
            <div class="har-sum-lbl">识别接口</div>
          </div>
          <div class="har-sum-item">
            <div class="har-sum-num">{{ harPreview.stats.skipped_static }}</div>
            <div class="har-sum-lbl">跳过静态资源</div>
          </div>
          <div class="har-sum-item">
            <div class="har-sum-num">{{ Object.keys(harPreview.stats.groups || {}).length }}</div>
            <div class="har-sum-lbl">业务分组</div>
          </div>
          <div class="har-sum-item">
            <div class="har-sum-num" :class="harPreview.stats.auth_injected ? 'ok' : 'warn'">
              {{ harPreview.stats.auth_injected ? '已补齐' : '未补齐' }}
            </div>
            <div class="har-sum-lbl">认证状态</div>
          </div>
        </div>

        <!-- 认证告警 -->
        <el-alert v-if="!harPreview.stats.auth_injected" type="warning" :closable="false" class="har-alert"
                  title="HAR 中不含认证凭据"
                  description="浏览器导出时会剥离 Authorization 头。请在「认证补齐」中配置自动登录取 token，否则导入的接口无法直接回放。" />

        <el-alert v-if="harPreview.stats.auth_error" type="error" :closable="false" class="har-alert"
                  :title="'认证获取失败：' + harPreview.stats.auth_error" />

        <!-- 动态参数 -->
        <div v-if="harDependencyList.length" class="har-block">
          <div class="har-block-title">
            <el-icon><WarningFilled /></el-icon>
            需动态取值的参数（{{ harDependencyList.length }} 个）
          </div>
          <div class="har-block-sub">
            这些参数随环境或账号变化，硬编码会导致取到错误数据却不报错，已替换为变量占位
          </div>
          <el-table :data="harDependencyList" size="small" border class="har-table">
            <el-table-column prop="key" label="参数名" width="180" />
            <el-table-column prop="var" label="变量" width="140" />
            <el-table-column prop="where" label="位置" width="90" />
            <el-table-column label="影响接口" width="100">
              <template #default="scope">{{ scope.row.used_by.length }}</template>
            </el-table-column>
        ...
        <div class="har-block">
          <div class="har-block-title">
            ...
            接口清单（{{ harPreview.apis.length }}）
          </div>
          <el-table :data="harPreview.apis" size="small" border max-height="420" class="har-table">
            <el-table-column prop="seq" label="#" width="50" />
            <el-table-column prop="group" label="分组" width="130" show-overflow-tooltip />
            <el-table-column prop="method" label="方法" width="80">
              <template #default="scope">
                <span class="har-method" :class="'m-' + scope.row.method.toLowerCase()">{{ scope.row.method }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="path" label="路径" min-width="240" show-overflow-tooltip />
            <el-table-column label="状态" width="70">
              <template #default="scope">
                <span :class="scope.row.status === 200 ? 'har-st-ok' : 'har-st-bad'">{{ scope.row.status }}</span>
              </template>
            </el-table-column>
            <el-table-column label="动态参数" width="200">
              <template #default="scope">
                <el-tag v-for="(v, k) in scope.row.body_dynamic" :key="'b' + k" size="small" type="warning"
                        class="har-tag">{{ harVarLabel(k, v) }}</el-tag>
                <el-tag v-for="(v, k) in scope.row.query_dynamic" :key="'q' + k" size="small" type="warning"
                        class="har-tag">{{ harVarLabel(k, v) }}</el-tag>
                <span v-if="!Object.keys(scope.row.body_dynamic || {}).length && !Object.keys(scope.row.query_dynamic || {}).length"
                      class="har-none">—</span>
              </template>
            </el-table-column>
          </el-table>
        ...
        <!-- 认证探测建议 -->
        <div v-if="harPreview.authSuggestion && harPreview.authSuggestion.url" class="har-block">
          <div class="har-block-title">
            <el-icon><Key /></el-icon>
            自动探测到的登录接口
          </div>
          <div class="har-sug">
            <div class="har-sug-row"><span class="k">地址</span><span class="v">{{ harPreview.authSuggestion.method }} {{ harPreview.authSuggestion.url }}</span></div>
            <div class="har-sug-row"><span class="k">Token 路径</span><span class="v">{{ harPreview.authSuggestion.tokenPath }}</span></div>
            <el-button size="small" type="primary" plain @click="applyAuthSuggestion" style="margin-top:8px">
              填入认证配置
            </el-button>
          </div>
        </div>
...
export default{
  ...
  computed:{
    ...
    // HAR 预览：把依赖对象转成表格行
    harDependencyList(){
      const deps = (this.harPreview && this.harPreview.dependencies) || {}
      return Object.keys(deps).map(k => {
        const d = deps[k]
        return {
          key: d.key,
          var: d.var,
          where: d.where === 'body' ? '请求体' : (d.where === 'query' ? '查询串' : '—'),
          reason: d.reason,
          nullable: d.nullable,
          used_by: d.used_by || [],
        }
    ...
    test_active_response_tab: 'ResponseStatus',
      ...
      importFormats: [
        ...
        },
        {
          ...
          desc: '浏览器录制，自动补齐认证',
          // ★ 2026-09-17 修复图标不显示：
          //   原 data URI 里最后那条「时钟指针」path 写法畸形
          //   （d="M41 47v5l4 2" 采用了折线但未闭合、且与前景圆叠加、属性顺序混乱），
          //   整个 SVG 解析失败 → <img> 渲染成破图，用户看到「HAR 抓包图标显示错误」。
          //   这里换成结构简单、只用 rect+path 的抓包图标（紫色底 + 白色向下箭头落入托盘），
          //   语法必然正确，且与其它 9 个格式卡片的视觉语言一致（圆角方块 + 白色图形）。
          iconSvg: 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"%3E%3Crect width="64" height="64" rx="12" fill="%237c3aed"/%3E%3Cpath fill="white" d="M32 14l10 12h-6v10h-8V26h-6l10-12z"/%3E%3Cpath fill="white" d="M16 42h32v4H16v-4zm0 6h20v4H16v-4z"/%3E%3C/svg%3E'
        },
      ],
  ...
  methods:{
	...
	async getAllServiceModule(){
		try {
			...
			const res = await this.$api.getAllServiceModule({project: pid})
			if (res.status === 200){
				this.serviceModuleTree = (res.data && res.data.results) || []
				// 空且无项目上下文 → 静默尝试自动恢复一次，再重查
				if (!this.serviceModuleTree.length && !this.liveProjectId()) {
					const r = await this.resolveProjectOrWarn({ silent: true })
					pid = this.liveProjectId()
					if (r.ok && pid) {
						const retry = await this.$api.getAllServiceModule({project: pid})
				...
				if (!this.serviceModuleTree.length && pid) {
					const retry2 = await this.$api.getAllServiceModule({project: pid})
					if (retry2.status === 200) this.serviceModuleTree = (retry2.data && retry2.data.results) || []
				}
			}
		} catch (error) {
    ...
    async import_api(){
      this.$refs['exportRef'].validate(async (valid, fields)=>{
        if(valid){
          this.apiExportSet.project = this.liveProjectId()
    ...
    onSelectImportFormat(fmt){
      this.apiImportSet.format = fmt
      // HAR 与 JMeter/OpenAPI 的差异：只有文件一种来源（浏览器导出），
      // 且需要先把解析结果看一遍再导入，因此强制切到文件源
      if(fmt === 'har'){
        this.apiImportSet.source = 'file'
        this.apiImportSet.url = ''
      }
    },
    onImportFileChange(file, fileList){
      this.importFileList = fileList.slice(-1)
      this.apiImportSet.file = file && file.raw ? file.raw : null
      // 换文件后旧预览失效，避免拿上一次的解析结果误判
    ...
    resetImportDialog(closeDialog = true){
      ...
      // HAR 状态一并重置，避免下一次打开沿用上次的认证配置与预览
      this.harAuth = {
        mode: 'login',
        token: '',
        headerName: 'Authorization',
        template: 'Bearer {token}',
        login: { url: '', method: 'POST', json: '', tokenPath: 'data.token' },
      }
      this.harPreview = null
      this.harPreviewVisible = false
      this.harPreviewLoading = false
    ...
    async submitImportV2(){
      ...
      try {
        ...
        if(this.apiImportSet.format === 'curl'){
          ...
          // HAR：附带认证补齐配置（未配置时不传，后端按不补齐处理）
          if(this.apiImportSet.format === 'har'){
    ...
    // ---------------- HAR 导入 ----------------
    harVarLabel(key, varName){
      return key + ' → ${' + varName + '}'
    },
    buildHarAuthConfig(){
      // mode=none 时不传 authConfig，后端不会尝试补齐
      if(this.harAuth.mode === 'none') return null
      const cfg = {
        headerName: this.harAuth.headerName || 'Authorization',
        template: this.harAuth.template || '{token}',
      }
      if(this.harAuth.mode === 'token'){
        if(!this.harAuth.token) return null
        cfg.token = this.harAuth.token
      } else {
        if(!this.harAuth.login.url) return null
        let loginJson = null
        const raw = (this.harAuth.login.json || '').trim()
        if(raw){
          try {
            loginJson = JSON.parse(raw)
          } catch (e) {
            ElMessage({ type: 'warning', message: '登录请求体不是合法 JSON，已忽略该字段' })
            loginJson = null
          }
        }
        cfg.login = {
          url: this.harAuth.login.url,
          method: this.harAuth.login.method || 'POST',
          json: loginJson,
          tokenPath: this.harAuth.login.tokenPath || 'data.token',
        }
      }
      return cfg
    },
    async previewHar(){
      ...
      try {
        ...
        const response = await this.$api.har_preview(payload)
        const respBody = response.data || {}
        ...
        if(response.status === 200 && inner && inner.apis){
          ...
          ElMessage({ type: 'error', message: inner.detail || respBody.detail || '解析失败' })
        }
      } catch (e) {
        ElMessage({ type: 'error', message: '解析异常：' + (e && e.message ? e.message : e) })
      } finally {
        this.harPreviewLoading = false
      }
    },
    applyAuthSuggestion(){
      const s = this.harPreview && this.harPreview.authSuggestion
      if(!s || !s.url) return
      this.harAuth.mode = 'login'
      this.harAuth.login.url = s.url
      this.harAuth.login.method = s.method || 'POST'
      this.harAuth.login.tokenPath = s.tokenPath || 'data.token'
      this.harAuth.headerName = s.headerName || 'Authorization'
      this.harAuth.template = s.template || 'Bearer {token}'
      if(s.json && !this.harAuth.login.json){
    ...
    async getServices(){
      const response = await this.$api.getServices({project: this.liveProjectId()})
      if (response.status === 200){
        this.service_list = {...response.data}
      }
    },
    async getPlantModule(){
      const response = await this.$api.getAllPlantModule({project: this.liveProjectId()})
      if (response.status === 200){
        this.plant_module_list = (response.data && response.data.results) || []
      }
    },
    async getApis(){
      this.apiSearch.project = this.liveProjectId()
   ...
   async handleAutoView() {
    ...
    async getEnvs(){
      const response = await this.$api.getEnvs({project: this.liveProjectId()})
      if (response.status === 200){
    ...
    async getfuncs(){
      const response = await this.$api.getFuncs({project: this.liveProjectId()})
      if (response.status === 200){
    ...
    async getPlants() {
      const response = await this.$api.getPlants({ project: this.liveProjectId() })
      if (response.status === 200) {
  ...
  async created() {
    ...
    // 注意：不能只看 await 返回值 —— store 的 projectResolved 闩锁会让 resolveProject
    // 在「之前已尝试过」时立即返回 { ok:false }，而此刻 store 里可能已有 projectInfo
    // （由路由守卫或 App.vue 快照写入）。故以 store 现值裁决，取参统一用 liveProjectId()。
...
.har-sum-num.ok { color: #10b981; }
.har-sum-num.warn { color: #f59e0b; font-size: 17px; }
...
.m-get { background: rgba(16,185,129,0.14); color: #059669; }
.m-post { background: rgba(59,130,246,0.14); color: #2563eb; }
.m-put { background: rgba(245,158,11,0.16); color: #d97706; }
.m-delete { background: rgba(239,68,68,0.14); color: #dc2626; }
.m-patch { background: rgba(139,92,246,0.14); color: #7c3aed; }

.har-st-ok { color: #10b981; font-weight: 600; }
.har-st-bad { color: #ef4444; font-weight: 600; }
