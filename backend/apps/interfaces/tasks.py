"""
异步任务 - 接口文档导入（V2）
- import_api_doc_task: 后台解析 OpenAPI/Swagger/Postman/JMeter/cURL/HAR 文档并批量落库，
  完成后更新站内信任务状态并推送消息。
"""
import logging
...
def import_api_doc_task(task_id: str, format: str, source: str, service_id: int,
    ...
    try:
        ...
        if format == ImportApiTypeV2.Curl:
            ...
            elif format == ImportApiTypeV2.Har:
                stats = _parse_har_v2(content, service_id, user_id, uri_prefix, match_mode,
                                      target_module_id, auth_config=auth_config)
            else:
        ...
        _update_message(task_id, service_id, message_id, user, stats, error,
                        duration=(timezone.now() - start).total_seconds(), fmt=format)


def _update_message(task_id, service_id, message_id, user, stats, error, duration, fmt=''):
    ...
    try:
        ...
        if error:
            ...
            # HAR 导入：把认证补齐与动态参数情况一并回报，这两点直接决定
            # 导入出来的接口能不能跑通、换环境会不会静默取错数据
            if fmt == ImportApiTypeV2.Har and stats.get('har'):
                har = stats['har']
                if har.get('auth_injected'):
                    content += "\n认证: 已自动补齐请求头 ✅"
                elif har.get('auth_header_present_in_har'):
                    content += "\n认证: HAR 中已含认证头"
                else:
                    content += "\n认证: ⚠ 未补齐（HAR 不含凭据，接口需手工补 token）"
                others = [f"  - {k}（{v['reason']}）" for k, v in (stats.get('dependencies') or {}).items()]
                if others:
                    content += "\n\n需动态取值参数:\n" + "\n".join(others[:10])
