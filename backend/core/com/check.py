...
from core.com.common import get_module_functions
from core.com.common import formatter_log, replace_params_class_data, replace_all_func_value
...
def check_equal(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验字符串是否相等
    """
    if check_exp_value == check_act_value:
        return True
    elif raise_assert_error:
        raise AssertionError
    else:
        return False


def check_not_equal(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验字符串是否不相等
    """
    if check_exp_value != check_act_value:
...
def check_number_equal(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验数字是否相等
    """
    try:
...
def check_number_not_equal(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验数字是否不相等
    """
    try:
...
def check_number_gt(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验实际值是否大于预期值
    """
    try:
...
def check_number_ge(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验实际值是否大于等于预期值
    """
    try:
...
def check_number_lt(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验实际值是否小于预期值
    """
    try:
...
def check_number_le(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验实际值是否小于等于预期值
    """
    try:
...
def check_bool_equal(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验布尔值是否相等
    """
...
def check_bool_not_equal(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验布尔值是否不相等
    """
    if check_exp_value.upper() != check_act_value.upper():
...
def check_object_is_empty(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验数组或字典对象是否为空
    """
...
def check_object_is_not_empty(check_exp_value, check_act_value, raise_assert_error=True):
    """
    校验数组或字典对象是否不为空
    """
    if isinstance(check_act_value, str):
...
CHECK_FUNC_MAP = get_module_functions('core.com.check')


def loop_assert_by_check_list(check_list, case_params, case_logs_obj, raise_assert_error=True, result=True,
                              exp_key='exp_value', act_key='act_value', check_key='method', is_push_log=True):
    """
    循环校验列表返回校验结果
    """
    ...
    for index, check_obj in enumerate(check_list):
        ...
        if is_push_log:
            ...
            log_obj = {'title': formatter_log('INFO', check_title), 'value': check_info}
            case_logs_obj.logs[-1]['logs'].append(log_obj)
        check_result = check_func(check_exp_value, check_act_value, raise_assert_error=raise_assert_error)

        # 如果是条件判断, 则不会抛出异常, 继续循环判断条件
        if not raise_assert_error:
            if index:
                result = check_result
            else:
